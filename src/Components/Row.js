...
function Row({title, fetchUrl, isLargeRow}) {
    ...
    const [trailerUrl, setTrailerUrl] = useState(""); //whenever we click a movie, this will store the trailer URL
// useEffect is a snippet of code that runs on a condition / variable
    useEffect(() => { 
  ...
  return (
    <div className="row">
       ...
       {trailerUrl && (<LiteYouTubeEmbed
                id={trailerUrl} 
                title="A descriptive title for your video"
                adNetwork={false} 
            />
        )}
    </div>
